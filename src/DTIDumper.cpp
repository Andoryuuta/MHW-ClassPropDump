...
void PopulateClassRecordProperties(ClassRecord* cr,
                                   std::map<std::string, Mt::MtDTI*>* dtiMap);
...
void DTIDumper::ParseDTI() {
    ...
    hashTableAOB = SigScan::Scan(image_base,
                                 "48 8D ?? ?? ?? ?? 02 48 8D 0C C1 48 8B 01 48 "
                                 "85 C0 74 06 48 8D 48 28 EB F2");
    if (hashTableAOB == 0) {
        // Monster Hunter Stories 2 initial PC release:
        hashTableAOB = SigScan::Scan(
            image_base,
            "48 8D ?? ?? ?? ?? ?? 48 8D 0C C1 48 8B 01 48 85 C0 74 18");
        if (hashTableAOB == 0) {
...
std::vector<ClassRecord> DTIDumper::GetClassRecords() {
     ...
     *    retn
     */
    auto potential_getdti_aob_matches =
        SigScan::ScanAll(this->image_base, "48 8D 05 ?? ?? ?? ?? C3");
    spdlog::info("Getter AOB matches: 0x{0:x}",
                 potential_getdti_aob_matches.size());

    // Create a map of offsets used in the found getter functions, mapped to the
    // getter function address.
    ...
    for (size_t i = 0; i < potential_getdti_aob_matches.size(); i++) {
        ...
        lea_offset_to_getter_map[displ + potential_getdti_aob_matches[i] + 7] =
            potential_getdti_aob_matches[i];
    }
    spdlog::info("Generated gotten address <-> DTI getter map. Size: {0:X}",
                 lea_offset_to_getter_map.size());
    ...
    for (size_t i = 0; i < dtis.size(); i++) {
        if (lea_offset_to_getter_map.find((uint64_t)dtis[i]) !=
            lea_offset_to_getter_map.end()) {
            uint64_t get_dti_vf_address =
                lea_offset_to_getter_map[(uint64_t)dtis[i]];
            std::vector<uint64_t> found = SigScan::AlignedUint64ListScan(
                this->image_base, get_dti_vf_address, PAGE_READONLY);

            if (found.size() != 1) {
                // spdlog::info("Got {0} matches for vf {1}::GetDTI ptr:
                // 0x{2:X}", found.size(), dtis[i]->class_name,
                // lea_offset_to_getter_map[(uint64_t)dtis[i]]);
                ...
                // spdlog::info("Got single match for vf ptr->{0}::GetDTI
                // 0x{1:X}", dtis[i]->class_name, found[0]);
            }
        } else {
            spdlog::info("Missing {0}::GetDTI: 0x{1:X}", dtis[i]->class_name,
                         lea_offset_to_getter_map[(uint64_t)dtis[i]]);
            undetermined_dti.push_back(dtis[i]);
        }

        if (i % 100 == 0) {
            spdlog::info("DTI->vftable mapping progress: {0}/{1}", i,
                         dtis.size());
        }
    }
    spdlog::info("Found total dti <-> vftable mapping(s): {0}",
                 dti_to_vftable.size());
    spdlog::info("Undetermined dti <-> vftable mapping(s): {0}",
                 undetermined_dti.size());

    if (undetermined_dti.size() > 0) {
        spdlog::info(
            "Attempting to resolve undetermined vftable through instanciation");
        for (size_t i = 0; i < undetermined_dti.size(); i++) {
            spdlog::info("Trying to resolve: {0}",
                         undetermined_dti[i]->class_name);
        ...
        spdlog::info("New total dti <-> vftable mapping(s): {0}",
                     dti_to_vftable.size());
    }
    ...
    debug_file.open("dti_dumper_debug_log.txt",
                    std::ios::out | std::ios::trunc);

    std::vector<ClassRecord> class_records;
    for (auto const& [dti, vftable_addr] : dti_to_vftable) {
        debug_file << fmt::format(
                          "Getting records for DTI: {0}. Vftable: {1:X} \n",
                          dti->class_name, vftable_addr)
                   << std::flush;
        ...
        if (this->excluded_classes.find(dti->class_name) ==
            this->excluded_classes.end()) {
            PopulateClassRecordProperties(&record, &dtiMap);
        } else {
            spdlog::warn("Skipping property population of excluded class: {0}",
                         dti->class_name);
        }
...
this->excluded_classes.end()) {
                        ...
                        spdlog::warn("Skipping excluded class: {0}",
dti->class_name); continue;
                }
                ...
                __try
                {
                        ...
                        // Create a MtPropertyList to populate with the
MtObject::PopulatePropertyList function. Mt::MtPropertyList* propListInst =
nullptr;
                        __try {
                                auto mtPropListDTI = dtiMap["MtPropertyList"];
                                propListInst =
reinterpret_cast<Mt::MtPropertyList*>(mtPropListDTI->CtorInstance(malloc(mtPropListDTI->ClassSize())));
                        }
                        __except (EXCEPTION_EXECUTE_HANDLER)
                        {
                                spdlog::error("Error instanciating
MtPropertyList!"); Sleep(15000); continue;
                        }

                        // Create an instance of the object we are dumping the
properties of. Mt::MtObject* mtObj = nullptr;
                        __try {
                                mtObj =
reinterpret_cast<Mt::MtObject*>(dti->CtorInstance(malloc(dti->ClassSize())));
                        }
                        __except (EXCEPTION_EXECUTE_HANDLER)
                        {
                                spdlog::error("Got error instanciating {0}",
dti->class_name); continue;
                        }


                        if (mtObj == nullptr) {
                                spdlog::warn("{0}::DTI->NewInstance() ==
nullptr", dti->class_name); continue;
                        }

                        obj = (void*)mtObj;
                        obj_vftable = *(void**)mtObj;

                        // Populate our property list with the overridden
virtual function on the MtObject.
                        __try
                        {
                        ...
                        __except (EXCEPTION_EXECUTE_HANDLER)
                        {
                                spdlog::error("Got error populating property
list for {0}", dti->class_name); continue;
                        }


                        if (propListInst->first_prop != nullptr) {
                                for (Mt::MtProperty* prop =
propListInst->first_prop; prop != nullptr; prop = prop->next) {
...
lhs->GetFieldOffsetFrom((uint64_t)obj) < rhs->GetFieldOffsetFrom((uint64_t)obj);
                                });
                ...
                __except (EXCEPTION_EXECUTE_HANDLER)
                {
                        spdlog::error("Got SEH exception while printing
properties for {0}", dti->class_name);
                }

                class_records.push_back(ClassRecord{ dti, obj, obj_vftable,
properties });
        }
...
void PopulateClassRecordProperties(ClassRecord* cr,
    ...
    __try {
        // Create a MtPropertyList to populate with the
        // MtObject::PopulatePropertyList function.
        Mt::MtPropertyList* propListInst = nullptr;
        __try {
            auto mtPropListDTI = (*dtiMap)["MtPropertyList"];
            propListInst = reinterpret_cast<Mt::MtPropertyList*>(
                mtPropListDTI->NewInstance());
        } __except (EXCEPTION_EXECUTE_HANDLER) {
        ...
        // Populate our property list with the overridden virtual function on
        // the MtObject.
        __try {
            typedef bool(__thiscall * PopulatePropertyList_t)(
                Mt::MtObject*, Mt::MtPropertyList*);
            auto address = (uint64_t)cr->class_vftable + (3 * 8);
            PopulatePropertyList_t fp =
                (PopulatePropertyList_t)(*(uint64_t*)address);
            spdlog::info("Calling {0}::GetDTI vf ptr: {1:X}",
                         cr->dti->class_name, address);

            fp(nullptr, propListInst);
        } __except (EXCEPTION_EXECUTE_HANDLER) {
            spdlog::error("Got error populating property list for {0}",
                          cr->dti->class_name);
            return;
        }

        if (propListInst->first_prop != nullptr) {
            for (Mt::MtProperty* prop = propListInst->first_prop;
                 prop != nullptr; prop = prop->next) {
                cr->properties.push_back(prop);
            }
        }

        std::sort(cr->properties.begin(), cr->properties.end(),
                  [&](const auto& lhs, const auto& rhs) {
                      return lhs->GetFieldOffsetFrom((uint64_t)0) <
                             rhs->GetFieldOffsetFrom((uint64_t)0);
                  });
    } __except (EXCEPTION_EXECUTE_HANDLER) {
...
void DTIDumper::DumpToFile(std::string filename) {
    ...
    for (const auto& rec : class_records) {
        ...
        while (cur != nullptr && cur->parent != cur) {
            if (cur->class_name != nullptr &&
                std::strlen(cur->class_name) != 0) {
        ...
        try {
            // Output calculated CRC if it differs from the value in memory
            // (MtDTI constructor allows CRC overrides, though these appear to
            // be unused).
            uint32_t classNameJAMCRC =
                (get_cstr_crc(rec.dti->class_name) & 0x7FFFFFFF);
            if (rec.dti->crc_hash == classNameJAMCRC) {
                file << fmt::format(
                    "// {0} vftable:0x{1:X}, Size:0x{2:X}, CRC32:0x{3:X}\n",
                    rec.dti->class_name, (uint64_t)rec.class_vftable,
                    rec.dti->ClassSize(), rec.dti->crc_hash);
            } else {
                file << fmt::format(
                    "// {0} vftable:0x{1:X}, Size:0x{2:X}, CRC32:0x{3:X}, "
                    "CalcCRC:0x{4:X}\n",
                    rec.dti->class_name, (uint64_t)rec.class_vftable,
                    rec.dti->ClassSize(), rec.dti->crc_hash, classNameJAMCRC);
            }
            ...
            for (const auto& prop : rec.properties) {
                ...
                std::string typeAndName = fmt::format(
                    "{0} '{1}'", prop->GetTypeName(), prop->prop_name);
                // int64_t cappedOffset = prop->GetFieldOffset();
                int64_t cappedOffset =
                    prop->GetFieldOffsetFrom((uint64_t)rec.obj_instance);
                ...
                if (prop->IsArrayType()) {
                    if (prop->IsGetterSetter()) {
                        // Dynamic array
                        std::string varString = fmt::format(
                            "{0}[*]", typeAndName, prop->data.var.count);
                        file << fmt::format(
                                    "\t{0:<50}; // Offset:0x{1:X}, "
                                    "DynamicArray, Getter:0x{2:X}, "
                                    "Setter:0x{3:X}, GetCount:0x{4:X}, "
                                    "Reallocate:0x{5:X} CRC32:0x{6:X}, "
                                    "Flags:0x{7:X}\n",
                                    varString, cappedOffset,
                                    prop->data.gs.fp_get, prop->data.gs.fp_set,
                                    prop->data.gs.fp_get_count,
                                    prop->data.gs.fp_dynamic_allocation,
                                    prop->GetCRC32(), prop->GetFullFlags())
                             << std::flush;
                    } else {
                        // Static/fixed-length array
                        std::string varString = fmt::format(
                            "{0}[{1}]", typeAndName, prop->data.var.count);
                        file << fmt::format(
                                    "\t{0:<50}; // Offset:0x{1:X}, Array, "
                                    "CRC32:0x{2:X}, Flags:0x{3:X}\n",
                                    varString, cappedOffset, prop->GetCRC32(),
                                    prop->GetFullFlags())
                             << std::flush;
                    }
                } else if (prop->IsGetterSetter()) {
                    file << fmt::format(
                                "\t{0:<50}; // Offset:0x{1:X}, PSEUDO-PROP, "
                                "Getter:0x{2:X}, Setter:0x{3:X}, "
                                "CRC32:0x{4:X}, Flags:0x{5:X}\n",
                                typeAndName, cappedOffset,
                                (uint64_t)prop->data.gs.fp_get,
                                (uint64_t)prop->data.gs.fp_set,
                                prop->GetCRC32(), prop->GetFullFlags())
                         << std::flush;
                } else {
                    file << fmt::format(
                                "\t{0:<50}; // Offset:0x{1:X}, Var, "
                                "CRC32:0x{2:X}, Flags:0x{3:X}\n",
                                typeAndName, cappedOffset, prop->GetCRC32(),
                                prop->GetFullFlags())
                         << std::flush;
                }
            }
    ...
    for (const auto& rec : class_records) {
        ...
        try {
            file << fmt::format("('{0}', 0x{1:X}, 0x{2:X}),\n",
                                rec.dti->class_name,
                                (uint64_t)rec.class_vftable, (uint64_t)rec.dti);
...
void DTIDumper::DumpResourceInformation(std::string filename) {
    ...
    for (const auto& rec : class_records) {
        __try {
            ...
            // spdlog::info("is resource class? {0} - {1}", rec.dti->class_name,
            // isResourceClass);

            if (isResourceClass) {
                if (rec.class_vftable != nullptr) {
                    uint64_t addr =
                        *(uint64_t*)(((uint64_t)rec.class_vftable) + (8 * 6));
                    typedef char* (*GetResourceExtension_t)();
                    GetResourceExtension_t GetExt =
                        (GetResourceExtension_t)(addr);
                    char* ext = GetExt();
...
std::map<std::string, Mt::MtDTI*> DTIDumper::GetFlattenedDtiMap() {
    ...
    for (int bucketIndex = 0; bucketIndex < 256; bucketIndex++) {
        ...
        while (true) {
            // spdlog::info("DTI@{0:X} = {1}, CRC32:{2:x}", (uint64_t)dti,
            // dti->class_name, dti->crc_hash);
